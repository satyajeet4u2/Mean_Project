...
export class PostService {
  ...
  addPost(title: string, content: string, image: File) {
    ...
    this.http.post<{ message: string, post: PostModel }>('http://localhost:3000/api/posts', postData).subscribe(responseData => {
  ...
  updatePost(id: string, title: string, content: string, image: File | string) {
    ...
    this.http.put<{ imagePath: any, creator: string }>('http://localhost:3000/api/posts/' + id, postData).subscribe(response => {
